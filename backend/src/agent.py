...
from services import MyAgentsLLM, ToolAwareSimpleAgent, ToolRegistry
...
class ResearchGraphState(TypedDict):
    """LangGraph state for the deep research workflow."""
...
class DeepResearchAgent:
    ...
    def __init__(self, config: Configuration | None = None) -> None:
        """Initialise the coordinator with configuration, graph and shared tools."""
        ...
        # 必须放在 planner / summarizer / reporting 初始化之后。
        self._graph = self._build_graph()

    # ------------------------------------------------------------------
    # Public API
    # ------------------------------------------------------------------
    def _init_llm(self) -> MyAgentsLLM:
        """Instantiate MyAgentsLLM following configuration preferences."""
    ...
    def run_stream(self, topic: str) -> Iterator[dict[str, Any]]:
        ...
        当前版本暂时保留原来的 Queue + Thread 流式实现。
        非流式 run() 已经切换到 LangGraph。
        """
    ...
    def _build_graph(self):
        """Build the LangGraph workflow for non-streaming research."""
        graph = StateGraph(ResearchGraphState)

        graph.add_node("plan_todos", self._plan_todos_node)
        graph.add_node("select_task", self._select_task_node)
        graph.add_node("execute_task", self._execute_task_node)
        graph.add_node("advance_task", self._advance_task_node)
        graph.add_node("write_report", self._write_report_node)
        graph.add_node("persist_report", self._persist_report_node)

        graph.add_edge(START, "plan_todos")
        graph.add_edge("plan_todos", "select_task")
        graph.add_edge("select_task", "execute_task")
        graph.add_edge("execute_task", "advance_task")
    ...
    def _plan_todos_node(self, graph_state: ResearchGraphState) -> dict[str, Any]:
        """Plan TODO tasks and create a fallback task when planning returns empty."""
        state = graph_state["summary_state"]

        state.todo_items = self.planner.plan_todo_list(state)
        events = self._drain_tool_events(state, step=0)
        ...
        for index, task in enumerate(state.todo_items, start=1):
            if not getattr(task, "stream_token", None):
    ...
    def _select_task_node(self, graph_state: ResearchGraphState) -> dict[str, Any]:
        """Select the current task by current_task_index."""
        state = graph_state["summary_state"]
        index = graph_state["current_task_index"]

        if index >= len(state.todo_items):
            return {
                "summary_state": state,
                "current_task": None,
                "events": [],
    ...
    def _execute_task_node(self, graph_state: ResearchGraphState) -> dict[str, Any]:
        """Execute search and summarization for the selected task."""
        state = graph_state["summary_state"]
        task = graph_state["current_task"]
        index = graph_state["current_task_index"]
    ...
    def _advance_task_node(self, graph_state: ResearchGraphState) -> dict[str, Any]:
        """Move to the next task."""
        return {
            "current_task_index": graph_state["current_task_index"] + 1,
            "current_task": None,
            "events": [],
        }

    def _route_after_task(self, graph_state: ResearchGraphState) -> Literal["continue", "report"]:
        """Route to the next task or report generation."""
        state = graph_state["summary_state"]
        current_task_index = graph_state["current_task_index"]

        if current_task_index < len(state.todo_items):
            return "continue"

        return "report"

    def _write_report_node(self, graph_state: ResearchGraphState) -> dict[str, Any]:
        """Generate final report after all tasks are executed."""
        state = graph_state["summary_state"]
        report = self.reporting.generate_report(state)

        final_step = len(state.todo_items) + 1
        events = self._drain_tool_events(state, step=final_step)
    ...
    def _persist_report_node(self, graph_state: ResearchGraphState) -> dict[str, Any]:
        """Persist final report into note storage if note tool is enabled."""
        state = graph_state["summary_state"]
